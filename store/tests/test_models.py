...
class TestAdresseesModel(TestCase):
    def setUp(self):
        self.data1 = Adressee.objects.create(name="django", slug="django")

    def test_category_model_entry(self):
        """
        Test adressee model data insertion/types/field attributes
        """
        data = self.data1
        self.assertTrue(isinstance(data, Adressee))
        self.assertEqual(str(data), "django")

    def test_adressee_url(self):
        """
        Test adressee model slug and URL reverse
        """
        data = self.data1
        response = self.client.post(reverse("store:adressee_list", args=[data.slug]))
